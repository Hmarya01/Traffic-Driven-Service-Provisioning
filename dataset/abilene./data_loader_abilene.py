...
def data_loader(train_rate, seq_len, source):
  ...
  # Load data
  ori_data = np.loadtxt('source_data_abilene.csv', delimiter=',', skiprows = 1)
  # Reverse the time order
  #reverse_data = ori_data[::-1]
  source_data_temp=ori_data[range(0,6*800),source] # (0,2000) for Geant
  source_data_temp=np.asarray(source_data_temp).reshape(800,6)
